...
static void _THREAD_StopApplication(void) {
  ...
  SIM_Log("\n挂起应用程序");
  if (_SuspendCount) {
    _MessageBox1("应用程序已暂停 ...");
    return;
  }
  if (!_ghThread) {
    _MessageBox1("应用程序没有运行...");
    return;
  }
...
static void _THREAD_ContinueApplication(void) {
  ...
  SIM_Log("\n应用程序继续");
  if (!_SuspendCount) {
    _MessageBox1("应用程序未停止...");
    return;
  }
  if (!_ghThread) {
    _MessageBox1("应用程序未运行...");
    return;
  }
...
static int _WinMain_NoClean(HINSTANCE hInstance, 
  ...
  if (_ahBmpDevice[0] && (_xPosLCD >= 0)) {
    ...
    _hWndMain = CreateWindowEx(0, // Extended style -> Use WS_EX_TOPMOST if you want window to stay on top
                              ...
                              ((WinRect.right-WinRect.left+1)-bmpDevice.bmWidth)/2, 
							  ((WinRect.bottom-WinRect.top+1)-bmpDevice.bmHeight)/2,
							  bmpDevice.bmWidth, bmpDevice.bmHeight, 0, NULL, _hInst, NULL);
  } else {
